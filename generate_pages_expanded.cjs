...
const chapters = [
  ...
  { 
    title: "16.Fabrication", 
    content: "<h3>16.1 Material Processing</h3><p>The fabrication phase of the project necessitated the combination of both traditional subtractive manufacturing and modern rapid additive manufacturing techniques to achieve the desired tolerances.</p><h3>16.2 Hardware Fabrication</h3><h4>16.2.1 Subtractive Manufacturing (Laser Cutting)</h4><p>The main structural chassis and internal partition walls were constructed from robust 3mm acrylic sheets. These were precisely dimensioned and cut to size using a CO2 laser cutter to ensure perfectly tight joint tolerances. All exposed outer edges were subsequently sanded down to remove sharp burrs for safety.</p><h4>16.2.2 Additive Manufacturing (3D Printing)</h4><p>The critical internal diverter flap, which manages the physical routing of the waste, was custom modeled and 3D printed using PLA filament. An internal hex-grid infill of 30% was utilized to maintain a remarkably high strength-to-weight ratio. Finally, sensor mounting brackets were custom bent from thin aluminum strips for adjustable angular mounting.</p>" 
  },
  { 
    title: "17.Assembly", 
    content: "<h3>17.1 Mechanical Assembly Sequence</h3><p>Physical assembly was methodically conducted in distinct chronological stages. First, the 3D-printed diverter flap was directly coupled to the splined shaft of the MG995 servo using a specialized structural circular horn, firmly secured with a central M3 machine screw. The servo body was then bolted securely to the main acrylic chassis acting as the anchor point.</p><h3>17.2 Sensor and Circuit Integration</h3><h4>17.2.1 Angular Sensor Positioning</h4><p>Next, the upper detection chamber was mated to the base. The IR, inductive, and moisture sensors were glued and bolted into their respective slotted brackets, carefully angled to perfectly intersect the anticipated drop trajectory of typical waste items.</p><h4>17.2.2 Electronics Harnessing</h4><p>The discrete electronic components were permanently soldered onto a custom pre-tinned perfboard, effectively forming a single robust shield that plugs directly into the Arduino Uno headers. All external sensor and motor wire harnesses were routed cleanly along the inner walls and anchored with zip ties to prevent hazardous interference with the moving diverter flap.</p>" 
  },
  { 
    title: "18.Testing & Results", 
    content: "<h3>18.1 Testing Methodology</h3><p>Testing protocols were strictly divided into localized component-level unit testing followed subsequently by rigorous full-system integration operational testing to guarantee reliability.</p><h3>18.2 Calibration Phase</h3><h4>18.2.1 Sensor Threshold Tuning</h4><p>Unit testing primarily verified and locked in the analog calibration thresholds deep within the code. The inductive sensor correctly and repeatedly identified aluminum and steel cans at an operational distance of 4mm. Simultaneously, the analog moisture sensor successfully differentiated between wet organic matter (like a wet paper towel) and dry plastics based on tuned voltage thresholds.</p><h3>18.3 System Performance Results</h3><p>During final integration testing, a controlled sample set encompassing exactly 100 mixed objects (30 metallic, 40 wet organic, 30 dry non-metallic) was introduced into the top intake. The system proudly and successfully segregated 92 out of 100 total objects. Errors primarily occurred with unordinarily heavily crumpled dry paper falsely snagging and triggering the metallic sensor. The overall calculated system accuracy was proven to be 92%.</p>" 
  },
  { 
    title: "19.Prototype Evaluation", 
    content: "<h3>19.1 Identified Strengths</h3><p>Evaluating the prototype post-testing confirmed that it effectively met its primary technical objectives, demonstrating unexpectedly rapid and reliable physical segregation. The total response time—from initial object insertion to the final flap actuation and reset—averaged a mere 1.2 seconds, falling well within highly acceptable usability limits for public bin deployments.</p><h3>19.2 Target Areas for Improvement</h3><h4>19.2.1 Physical Durability Concerns</h4><p>While the structural integrity of the acrylic chassis proved highly adequate for light-to-medium indoor waste, municipal or industrial usage would undoubtedly require heavy-duty sheet steel or impact-resistant thick polycarbonate panels to survive abuse.</p><h4>19.2.2 Sensor Maintenance Requirements</h4><p>The evaluation clearly highlighted that the capacitive moisture sensor, due to its contact-based nature, required unacceptably frequent cleaning to maintain reading accuracy over long durations, as wet debris occasionally clung to it. Upgrading to a specialized non-contact optical spectrometer is highly recommended for commercial iterations.</p>" 
  },
  { 
    title: "20.Cost & Financial Feasibility", 
    content: "<h3>20.1 Prototype Manufacturing Costs</h3><p>Maintaining extreme affordability was a foundational project priority. The completed Bill of Materials (BoM) for the singular prototype is strictly tabulated as follows:<br/><br/>- Core Config: Arduino Uno (₹500)<br/>- Sensor Node: IR, Inductive, Moisture pack (₹400)<br/>- Actuator: MG995 Servo Motor (₹350)<br/>- Structural: Acrylic panels & Fastener Hardware (₹600)<br/>- Distribution: Perfboard, Electronics & Power unit (₹300)<br/><br/><strong>Total Bare-Metal Prototype Cost: ₹2,150.</strong></p><h3>20.2 Scalability and Mass Production</h3><h4>20.2.1 Manufacturing Economics at Scale</h4><p>When strategically scaling this design to mass production levels (e.g., runs of 10,000 units), economies of scale combined with custom integrated PCB fabrication and plastic injection molding will drastically lower the per-unit subassembly cost to an estimated ₹1,200.</p><h4>20.2.2 Broader Market Context</h4><p>Given that standard, un-mechanized municipal tier waste bins currently cost organizations roughly ₹1,500 to ₹3,000 to procure, adding highly functional smart sorting capabilities for effectively the same baseline price point demonstrates exceptionally high financial feasibility and profound market viability.</p>" 
  },
  { 
    title: "21.Applications", 
    content: "<h3>21.1 Micro Deployments and Campuses</h3><h4>21.1.1 Smart Educational Campuses</h4><ul><li>Direct deployment in progressive educational institutions, universities, and corporate tech parks seeking to actively automate and enforce their internal zero-waste initiatives without relying on honor systems.</li></ul><h4>21.1.2 Specialized Healthcare Facilities</h4><ul><li>Utilizing heavily re-calibrated hardware for intelligently segregating standard biological waste streams from clean pharmaceutical packaging plastics in busy clinical zones.</li></ul><h3>21.2 Macro Deployments</h3><ul><li><strong>Public Transportation Hubs:</strong> Deployment inside fast-paced airports and dense railway stations where consistently high foot-traffic volumes predictably result in massive quantities of poorly sorted and highly contaminated waste bins.</li><li><strong>Municipal Street Infrastructure:</strong> Upgrading core public infrastructure at the curb to aggressively pre-sort materials before they ever physically reach centralized waste processing sorting plants.</li></ul>" 
  },
  { 
    title: "22.Stakeholders", 
    content: "<h3>22.1 Direct System Beneficiaries</h3><h4>22.1.1 The General End Users</h4><p>The general public, students, and busy corporate employees who regularly deposit waste. The system offers them a completely seamless, guilt-free disposal experience by entirely removing the cognitive load and guesswork typically required in manual multi-bin sorting systems.</p><h4>22.1.2 Institutional Management Teams</h4><p>Schools, universities, and corporate entities deploying these bins trivially fulfill their strict sustainability goals, definitively improve general campus hygiene, and indirectly boost their public ESG (Environmental and Social Governance) ratings with stakeholders.</p><h3>22.2 Systemic Beneficiaries</h3><p><strong>Local Municipal Corporations</strong> fundamentally benefit from massive, compounding cost reductions in their centralized sorting facilities and a significantly extended lifespan of their local, finite landfills. Secondly, private <strong>Waste Management and Recycling Companies</strong> directly receive pre-sorted, high-purity raw materials at no extra sorting cost, boosting margins.</p>" 
  },
  { 
    title: "23.Advantages", 
    content: "<h3>23.1 Technical and Economic Advantages</h3><h4>23.1.1 Extreme Precision Vs. Cost</h4><ul><li><strong>High Precision vs Cost Ratio:</strong> Astoundingly achieves &gt;90% accuracy utilizing a cobbled-together sensor array that phenomenally costs less than ₹1000 at retail pricing.</li><li><strong>Ultra-Low Power Consumption:</strong> Because the system idles deeply, it can easily be continuously powered indefinitely by a small, cheap solar panel and integrated lithium battery setup, natively enabling off-grid municipal deployment.</li></ul><h3>23.2 Operational Advantages</h3><ul><li><strong>Fully Automated & Contactless:</strong> Completely, safely removes the fragile human element from the deeply hazardous and unsanitary task of sorting inherently mixed, potentially sharp waste items.</li><li><strong>Real-time Operation Metrics:</strong> Processes, analyzes, and mechanically sorts dropped waste in under 1.5 seconds flat, forcefully preventing physical bottlenecks at busy high-traffic disposal points.</li></ul>" 
  },
  { 
    title: "24.Limitations", 
    content: "<h3>24.1 Mechanical Hardware Limits</h3><h4>24.1.1 Throughput and Jam Constraints</h4><p>Despite its resounding successes, the prototype has inherent physical limitations. The system forcefully processes items serially; therefore, if an impatient user dumps a large, tightly bound cluster of mixed waste simultaneously, the mechanical diverter flap may physically jam, or worse, sort the entire diverse cluster based incorrectly on a single leading sensor reading.</p><h3>24.2 Sensor Shortcomings</h3><h4>24.2.1 Physical Contact Limitations</h4><p>Furthermore, the standard capacitive soil moisture sensor heavily relies on direct, physical surface contact to measure dielectric changes. This means deeply wet organic materials may stubbornly stick to it during operation, absolutely requiring period manual cleaning by janitorial staff to ensure sensors remain unblinded.</p><h4>24.2.2 Material Resolution Boundaries</h4><p>Finally, the exceedingly basic current sensor array cannot confidently distinguish between more complex sub-categories of dry waste, such as perfectly differentiating a glass bottle from a thick, clear plastic bottle without adding exorbitant optical classification hardware.</p>" 
  },
  { 
    title: "25.Social & Environmental impact", 
    content: "<h3>25.1 Profound Ecological Advantages</h3><p>By heavily and accurately separating wet, corrosive organic waste from dry, delicate recyclables at the immediate source of discard, the system outright prevents the catastrophic cross-contamination of paper fibers and recyclable plastics that ruins entire batches.</p><h4>25.1.1 Direct Emission Reduction</h4><p>This pristine separation allows wet organic waste to be smoothly diverted to local composting or biogas facilities, maximizing dry recycling yields and significantly, measurably reducing toxic methane emissions emanating from anaerobic landfill decomposition.</p><h3>25.2 Societal Upliftment</h3><p>Urban sanitation work is historically dangerous, socially degrading, and underpaid. Automating the most hazardous sorting phase elevates global occupational safety and actively dignifies waste management labor. It catalyzes a technological shift by promoting human roles upwards toward dignified system maintenance, auditing, and logistics routing rather than dangerous hand-picking.</p>" 
  },
  { 
    title: "26.Future Scope", 
    content: "<h3>26.1 Next-Generation Hardware</h3><h4>26.1.1 Deep IoT and Cloud Architecture</h4><p>Future iterations of the platform will aggressively focus on natively integrating IoT communication capabilities. Utilizing a slightly more powerful ESP32 microcontroller, deployed bins could automatically transmit real-time fill-levels, operational health, and localized sorting statistics to a centralized municipal cloud dashboard via persistent Wi-Fi or long-range LoRaWAN networks.</p><h3>26.2 Algorithmic Upgrades</h3><h4>26.2.1 Embedded Machine Learning at the Edge</h4><p>Powerful new Machine Learning models, highly optimized to run continuously on low-power edge devices (such as a Raspberry Pi married to an optical camera module), could be deployed to replace or heavily augment the physical sensor suite. This bold step would allow the system to flawlessly classify specific sub-grades of plastics (identifying PET bottles versus HDPE containers) with unprecedented accuracy.</p>" 
  },
  { 
    title: "27.Conclusion", 
    content: "<h3>27.1 Comprehensive Project Summary</h3><p>The exhaustive Design and Development cycle of the prototype Smart Waste Segregation System irrefutably validates that ultra-low-cost, decentralized, and fully automated point-of-source waste sorting is both technically feasible and highly financially viable for mass deployment.</p><h4>27.1.1 Performance Restatement</h4><p>By effectively and intelligently fusing standard inductive, capacitive, and infrared sensing technologies with incredibly reliable, gravity-assisted mechanical actuation logic, the prototype bravely achieved a highly impressive, repeatable 92% segregation accuracy rate during mixed-load stress testing.</p><h3>27.2 Final Concluding Remarks</h3><p>Ultimately, this targeted micro-project serves as a deeply critical, tangible proof-of-concept. It clearly demonstrates how intelligently applied microcontroller electronics and basic engineering mechanics can forcefully solve massive, localized environmental challenges. Boldly scaling such localized smart-bin systems natively possesses the disruptive, world-changing potential to fundamentally transform global municipal waste management paradigms.</p>" 
  },
  { 
    title: "28.Reference", 
